...
<%} %> --%>
						<div class="caption font-red-sunglo">
							<span class="caption-subject bold "> Mapping Between Location & Validation
							</span>
						</div>
								...
								<div class="row" id="template_data">
									<div class="col-md-6 col-capture">
										<div class="form-group form-md-line-input">
											<div class="form-group form-md-line-input">
											<select class="form-control" id="locationsid" name="location">
											<option disabled selected value> -- select an option -- </option>
      <c:forEach var="myMap" items="${locationList}">
        <option value="${myMap.key}"><c:out value="${myMap.value}"/></option>
      </c:forEach>
    </select> <label for="form_control_1" id="host-id">Select Location*</label>
										</div>
										</div>
									...
									<div class="col-md-6 col-capture">
										<div class="form-group form-md-line-input">
											...
											<div class="form-group form-md-line-input">
											<select class="form-control" id="validationid" name="validation">
											<option disabled selected value> -- select an option -- </option>
      <c:forEach var="myMap" items="${ValidationList}">
        <option value="${myMap.key}"><c:out value="${myMap.value}"/></option>
      </c:forEach>
    </select> <label for="form_control_1" id="host-id">Select Validation*</label>
										</div>									</div>
									</div>
								</div>
								<div class="row" id="template_data">
									<div class="col-md-6 col-capture">
										<div class="form-group form-md-line-input">
											<div class="checkbox">
											  <label><input type="checkbox" id="check-existingValidation-id" value="">Select Relation</label>
											</div>
										</div>
										<span id="amit" class="help-block required"></span>
									</div>
									<div class="col-md-6 col-capture">
										<div class="form-group form-md-line-input">
											<div class="form-group form-md-line-input" id="existingValidation-div-id">
											<select class="form-control" id="existingValidationid" name="existingValidation">
											<option disabled selected value> -- select an option -- </option>
      <c:forEach var="myMap" items="${ValidationList}">
        <option value="${myMap.key}"><c:out value="${myMap.value}"/></option>
      </c:forEach>
    </select> <label for="form_control_1" id="lblexistingValidationid">Select Existing Validation*</label>
										</div>
										</div>
									</div>
								</div>
					<div class="form-actions noborder align-center">
						<button 
							onclick="return insertLocationRecord()" class="btn blue">Create<br/></button>
...
<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		$("#existingValidation-div-id").hide(); // on page load hide existing validations
...
<script type="text/javascript">
	...
	var globalObjData = null;
	
	// hide and show existing validion on click of check box
	$("#check-existingValidation-id").click(function() {
	    if($(this).is(":checked")) {
	        $("#existingValidation-div-id").show(300);
	    } else {
	        $("#existingValidation-div-id").hide(200);
	    }
	});
	

	/* last update : 12thApril2020
		Code By : Anant Mahale
		1. Insert location to validation mapping and validation to validation mapping.
	*/
function insertLocationRecord(){
	
	console.log("addNewLocation : insertLocationRecord");
	
	var strSelectedLocation = "";
	var strSelectedValidation = "";
	var strSelectedExistingValidation = "";
	var strErrorMsg = "";
	
	// validation to check location name is selected or not
	var  varLocationSelect = document.getElementById('locationsid'); // or in jQuery use: select = this;
	if (varLocationSelect.value) {
		strSelectedLocation = $("#locationsid").val();
	 	console.log(" value selected for location");
	}else{
		strErrorMsg = strErrorMsg + 'Please Select Location ';
	}
	
	// validation to check validation is checked or not
	var  varValidationSelect = document.getElementById('validationid'); // or in jQuery use: select = this;
	if (varValidationSelect.value) {
		strSelectedValidation = $("#validationid").val();
	 	console.log(" value selected for validation");
	}else{
		if(strErrorMsg != ""){
			strErrorMsg = strErrorMsg + '<br>';
		}
		strErrorMsg = strErrorMsg + 'Please Select Validation';
	}
	
	// if check box checked then it will check for existing validation
	if($("#check-existingValidation-id").is(":checked")) {
	
		var  varValidationSelect = document.getElementById('existingValidationid'); // or in jQuery use: select = this;
		if (varValidationSelect.value) {
			strSelectedExistingValidation = $("#existingValidationid").val();
		 	console.log(" value selected for existing validation");
		}else{
			if(strErrorMsg != ""){
				strErrorMsg = strErrorMsg + '<br>';
			}
			strErrorMsg = strErrorMsg + 'Please Select Existing Validation ';
		}
	}
		
	if(strErrorMsg != ""){ // if error is not empty then print 
		 toastr.info(strErrorMsg);
	}else{
		$.ajax({
	        type: "POST",
	        headers: { 'token':$("#token").val()},
	        url: "./insertLocationMapping",
	        async: false,
	        data:{
	        	existingValidation : strSelectedExistingValidation,
	        	validation : strSelectedValidation,
	        	location :  strSelectedLocation
	        },
	        success: function (message) {
	        	var j_obj = $.parseJSON(message);
                if (j_obj.hasOwnProperty('success')) {
               	 toastr.info('New Mapping Inserted');
	        ...
	        error: function (textStatus, errorThrown) {
	        	toastr.info('Record not inserted. There was a problem.');
	        	resetSelectedOptions();
	        }
	    });
	}
	...
	function resetSelectedOptions(){
		$('#check-existingValidation-id').prop('checked', false); 
		$('#locationsid').prop('selectedIndex',0);
        $('#validationid').prop('selectedIndex',0);
        $('#existingValidationid').prop('selectedIndex',0);
	}
	...
	$('#projectid span').on('click', function(){
		...
		console.log("mapLocationAndValidation : insertLocationRecord :: "+globalProjectId);
		var strErrorMsg = "";
		if(globalProjectId == ""){
		...
		if(strErrorMsg != ""){
			...
			window.location.href = './mapLocationAndValidation?projectId='+globalProjectId;
		}
	});
</script>
</head>