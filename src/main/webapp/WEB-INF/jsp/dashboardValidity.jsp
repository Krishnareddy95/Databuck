...
<body>
	...
	<div class="portlet-title">
		<div class="caption font-red-sunglo">
			<span class="caption-subject bold ">Column Summary <a onClick="validateHrefVulnerability(this);javascript:downloadCsvReports('${idApp}','${ColSumm_TableName_Validity}','ValidityData')"
				 href="#"
	...
	<input type="hidden" id="tableName"
		value="${ColSumm_TableName_Validity}">
		...
		<table id="ColSummTable"
			...
			<thead>
				<tr>
					...
					<td>Action<a onClick="validateHrefVulnerability(this)"
						href='rejectAll?rejectAll=True&tableName=${ColSumm_TableName_Validity}&tab=Validity&idApp=${idApp}'>
			...
			<%-- <tbody>
										<c:forEach var="data_Quality_Column_SummaryListobj"  items="${data_Quality_Column_Summary}">
											...
											<tr>
												<td>${data_Quality_Column_Summaryobj.date}</td>
...
jQuery(document).ready(function() {
	...
	table = $('#ColSummTable').dataTable({
		 	...
		 	"dataSrc": "",
		 	'sScrollX' : true,
		 	"sAjaxSource" : path+"/ColSummTableForValidityTableTab?tableName="+tableName+"&idApp="+idApp,
            "aoColumns": [
                {"data" : "id"},
                {"data" : "Date"},
                {"data" : "Run"},
                ...
                {"data" : "Action",
                     ...
                     var Run =row.Run;
                        ...
                        if (data === "Rejected") {
                            ...
                            data = "<a href='rejectInd?dGroupCol=" + dgroupCol + "&tab=Validity&Date="
						+ Date + "&tableName=" + tableName + "&dGroupVal=" + dGroupVal
						+ "&Run=" + Run + "&idApp=" + idApp + "'>"
						+ "<i class='fa fa-thumbs-up'></i></a>" + "<style>.fa { font-size: 26px;}</style>";
                        }
...
<script>
    ...
    var form_data = {
    		validityRunFilter : $("#validityRunFilter").val()

    };
    alert($("#validityRunFilter").val());
    $.ajax({
        url : './runFilter',
        type : 'GET',
