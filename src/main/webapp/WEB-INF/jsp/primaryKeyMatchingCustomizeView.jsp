				...
				<div class="portlet light bordered init">
					<div class="portlet-title">
						<div class="caption font-red-sunglo">
							<span class="caption-subject bold "> Edit Validation Check (Primary Key Matching) </span>
						</div>
					</div>
							...
							<div class="col-md-6 col-capture">
								<div class="form-group form-md-line-input">
									<input type="text" class="form-control" id="SelectLeftDataTmpl"
										name="description" value="${idData}_${applicationName}" style="margin-left: -20px;width: 735px;" readonly /> <label
										for="form_control" style="margin-left: -17px;">Left Source Template </label>
								</div>
							</div>
								...
								<div class="form-group form-md-line-input">
									...
									<select class="form-control" id="SelectRightDataTmpl"
										...
										<c:forEach var="getlistdatasourcesnameobj"
											...
											<option value="${getlistdatasourcesnameobj.idData}" ${getlistdatasourcesnameobj.idData == secondSourceIdData ? 'selected' : ''}>${getlistdatasourcesnameobj.idData}_${getlistdatasourcesnameobj.name}</option>
										</c:forEach>
									</select>
									<input type="text" class="form-control" id="SelectRightDataTmpl1"
									                 name="description" value="${secondSourceIdData}_${secondSourceName}" readonly />
									 <label for="form_control_1">Right Source Template *</label>
							...
							<div id='divConfigBody'>
								...
								<div id='divConfigVisual' style='display: block;'>
									<table id="tblPrimaryKeyMatching" class="table table-striped table-bordered table-hover" width="100%">
										<thead>
											<tr>
												 <th>Select Primary Key</th>
												 <th>Select Match Field</th>
												 <th>Left Column</th>
												 <th>Left Type</th>
...
<script>
	var ManagePrimaryMatchingValidation = {
		...
		setup: function() {
            $('#SelectRightDataTmpl').on('change',ManagePrimaryMatchingValidation.mainEventHandler).trigger('change');
            $("#SelectRightDataTmpl").hide();
		...
		mainServerHandler: function(sEventContext) {
			...
			if (lCallServer) {
					JwfAjaxWrapper({
					WaitMsg: sWaitMsg,
					Url: 'editPrimaryMatchingHandler',
					Headers:$("#token").val(),
					Data: { sWrapperData: JSON.stringify(oWrapperData),idApp: JSON.stringify(${idApp})},
				...
				ManagePrimaryMatchingValidation.DataTable = $(sViewTable).DataTable ({
					...
					drawCallback: function( oSettings ) {
                        ...
                        ManagePrimaryMatchingValidation.RightDataReloaded = false;
                         var list1 =$.parseJSON(JSON.stringify(${jsongetMatchingRules}));
                              $.each(list1, function( index, value ) {
                                    $(value).each(function (index, item) {
                                        $('#RightColumnNameAndType-'+item.idLeftColumn).val(item.rightColumn).change();
                                        if(item.matchType=="PRIMARY_KEY_MATCH_JOIN_FIELD")
                                             {
                                                $('#LeftPrimaryColumnId-check-'+item.idLeftColumn).trigger('click');
                                             }
                                        else if(item.matchType=="PRIMARY_KEY_MATCH_VALUE_FIELD")
                                            {
                                              $('#LeftValueColumnId-check-'+item.idLeftColumn).trigger('click');
                                            }
                                        $('#LeftExprTextBox-'+item.idLeftColumn).val(item.leftSideExp);
                                        $('#LeftExprTextBox-'+item.idLeftColumn).trigger('change');
                                        $('#RightExprTextBox-'+item.idLeftColumn).val(item.rightSideExp);
                                        $('#RightExprTextBox-'+item.idLeftColumn).trigger('change');
                                        if (item.rightSideExp !== '' )
                                        {
                                          $('#RightCustomize-check-'+item.idLeftColumn).trigger('click');
                                        }
                                        else if(item.leftSideExp !== '')
                                        {
                                         $('#LeftCustomize-check-'+item.idLeftColumn).trigger('click');
                                        }
                                  });
                              });
					}
		...
		console.log("Template Id="+${secondSourceIdData});
